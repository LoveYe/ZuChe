...
@interface AccomplishViewController ()<UITableViewDataSource,UITableViewDelegate,MBProgressHUDDelegate>
{
    UIView *_lineLabel;
    NSArray * _arrArray;
    NSArray *_listArray;
    MBProgressHUD *HUD;
    UIImageView* imageView233;
    UILabel*tishi233;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    {
        if (imageView233==nil||tishi233==nil) {
            imageView233 =[[UIImageView alloc]initWithFrame:CGRectMake(ScreenWidth/2-ScreenWidth*0.55/2, ScreenHeight/2-ScreenWidth*0.4-ScreenWidth*0.17, ScreenWidth*0.55,ScreenWidth*0.55)];
            imageView233.image =[UIImage imageNamed:@"小车"];
            [self.view addSubview:imageView233];
            
            
            tishi233 =[[UILabel alloc]initWithFrame:CGRectMake(ScreenWidth/2-ScreenWidth*0.55/2, ScreenHeight/2-ScreenWidth*0.6+ScreenWidth*0.6, ScreenWidth*0.55, ScreenWidth*0.2)];
            tishi233.numberOfLines =0;
            tishi233.textAlignment =NSTextAlignmentCenter;
            [self label:tishi233 isBold:YES isFont:14];
            tishi233.textColor=Color(99,99, 99);
            tishi233.text= @"您还没有登录，请先登录!";
            [self.view addSubview:tishi233];
        }
...
-(void)Down
{
    if ([ZCUserData share].isLogin==YES)
    {
        ...
        [HttpManager arrayPostData:dict andUrl:[NSString stringWithFormat:@"%@api.php?op=zkorderlist&status=5",NowUrl] Arraysuccess:^(NSArray *arrayfanhui) {
...
- (void)viewDidLoad {
    ...
    {
        [imageView233 removeFromSuperview];
        [tishi233 removeFromSuperview];
...
//    label.text =[NSString stringWithFormat:@"共 %lu 辆车 总价:",(unsigned long)array.count];
//    
...
//    price.text =[NSString stringWithFormat:@"¥ %@",[[_arrArray objectAtIndex:section] objectForKey:@"zongji"]];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    UILabel * zhuangtai =[[UILabel alloc]initWithFrame: CGRectMake(ScreenWidth-ScreenWidth*0.31-ScreenWidth*0.08, 0, ScreenWidth*0.3, nowHeight)];
    ...
    if ([string isEqualToString:@"1"]) {
        ...
        zhuangtai.text  =@"未评价";
        zhuangtai.textColor=jiedancolor;
        [btn addTarget:self action:@selector(quPingJia:) forControlEvents:UIControlEventTouchUpInside];
        ...
        zhuangtai.text  =@"已评价";
        zhuangtai.textColor=shenghuicolor;
        [btn addTarget:self action:@selector(kanPingJia:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)quPingJia:(UIButton *)btn
{
    TijiaopingjiaViewController *vc =[[TijiaopingjiaViewController alloc]init];
    vc.orderid=[NSString stringWithFormat:@"%@",[[_arrArray objectAtIndex:btn.tag] objectForKey:@"orderid"]];
    vc.dingdanhaoID=[NSString stringWithFormat:@"订单号:%@",[[_arrArray objectAtIndex:btn.tag] objectForKey:@"dingdanhao"]];
    
    [self.navigationController pushViewController:vc animated:YES];
}
-(void)kanPingJia:(UIButton *)btn
{
