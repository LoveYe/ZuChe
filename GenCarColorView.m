...
@interface GenCarColorView()<UIPickerViewDataSource,UIPickerViewDelegate>
...
@implementation GenCarColorView
...
{
    GenCarColorView *mview = [[self alloc]initWithFrame:frame];
...
{
    //代理传值
    if ([_delegate respondsToSelector:@selector(getColorChoiceValues222:)]) {
        if (!self.colorStr) {
            self.colorStr = self.firstColor;
        }
        [_delegate getColorChoiceValues222:self.colorStr];
    }
    [self removeView];
}
//取消蒙层
