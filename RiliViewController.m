...
@interface RiliViewController ()<UICollectionViewDelegate, UICollectionViewDataSource>
{
    NSInteger _year;
    NSInteger _month;
    NSInteger _day;
...
- (void)viewDidLoad {
    ...
    NSArray *array = [self.bukezuTime componentsSeparatedByString:@","];
    
    for (int i = 0; i < array.count; i++) {
        
        NSString *str = array[i];
        NSDictionary *dictnary = [NSDictionary dictionary];
        NSArray *dicArray = [str componentsSeparatedByString:@"-"];
        dictnary = [NSDictionary dictionaryWithObjectsAndKeys:dicArray[0],@"year",dicArray[1],@"month",dicArray[2],@"day", nil];
        [bukezuArray addObject:dictnary];
    }
...
//    
//    NSArray *arr = [self.bukezuTime componentsSeparatedByString:@","];
//    
