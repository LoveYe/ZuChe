...
- (void)initWithProperty
{
    ...
    self.titleSelectColor = [UIColor colorWithRed:7/255.0 green:187/255.0 blue:177/255.0 alpha:1];
    self.titleFont = [UIFont fontWithName:@"Arial-BoldMT" size:15];
...
- (void)layoutSubviews
{
    ...
    if (totalBtnWidth <= CGRectGetWidth(self.bounds)) {//不能滑动
        CGFloat itemBtnWidth = CGRectGetWidth(self.bounds)/(self.itemBtnArr.count+4);
        ...
        [self.itemBtnArr enumerateObjectsUsingBlock:^(UIButton * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (idx ==0){
                 obj.frame = CGRectMake(1 * itemBtnWidth, 0, itemBtnWidth, itemBtnHeight);
            }else if (idx == 1) {
                 obj.frame = CGRectMake(3 * itemBtnWidth, 0, itemBtnWidth, itemBtnHeight);
            }else if (idx == 2) {
                 obj.frame = CGRectMake(5 * itemBtnWidth, 0, itemBtnWidth, itemBtnHeight);
            }
    ...
    [UIView animateWithDuration:(animated?0.05:0) animations:^{
        switch (_indicatorType) {
            case FSIndicatorTypeDefault:
                ...
                self.indicatorView.bounds = CGRectMake(0, 0, 10, 2);
                break;
            ...
            case FSIndicatorTypeCustom:
                self.indicatorView.center = CGPointMake(selectBtn.center.x, CGRectGetHeight(self.scrollView.bounds) - 1);
                self.indicatorView.bounds = CGRectMake(0, 0, 10 + _indicatorExtension*2, 2);
                break;
