...
@interface FwzMapAnniviewViewController ()<MAMapViewDelegate,AMapLocationManagerDelegate> {
    ...
    UILabel *_timeLabel;
    UILabel *_gongliLabel;
    UILabel *_chaoTimeLabel;
...
@implementation FwzMapAnniviewViewController
...
-(AliMapViewCustomPinAnnotationView *)NotfullCustomAnnotationViewWithMapView:(MAMapView *)mapView viewForAnnotation:(CustomPointAnnotation*)annotation{
    ...
    annotationView.image = [UIImage imageNamed:@"icon_nav_start.png"];
    //设置为NO，用以调用自定义的calloutView
    annotationView.canShowCallout = NO;
...
-(void)createPointAnnotation{
    ...
    telButton.frame = CGRectMake(wid/2-30,hei-60/2-64-49,60,60);
    [_mapView addSubview:telButton];
    
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(wid-110, 10, 100, 100)];
    ...
    for (int i = 0; i <8; i++) {
        if (i%2 == 0) {
            if (i==0 || i== 2) {
                 UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(10, 10+15*i/2, 20, 15)];
                if (i ==0) {
                     label.text = @"用时:";
            ...
            if (i==4 || i == 6) {
                 UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(10, 10+15*i/2+10, 20, 15)];
                if (i ==4) {
                    label.text = @"超时:";
                    label.adjustsFontSizeToFitWidth = YES;
                }else {
        ...
        if (i%2 == 1) {
            if (i==1 || i== 3) {
                if (i ==1) {
//                    label.text = @"超时:";
                    _timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(30, 13+15*(i-1)/2, 60, 15)];
                  ...
                  //   _timeLabel.backgroundColor = [UIColor greenColor];
                    ...
                    _gongliLabel = [[UILabel alloc] initWithFrame:CGRectMake(30, 13+15*(i-1)/2, 60, 15)];
                    _gongliLabel.adjustsFontSizeToFitWidth = YES;
                    _gongliLabel.font = [UIFont systemFontOfSize:14];
                 //   _gongliLabel.backgroundColor = [UIColor greenColor];
                 _gongliLabel.textColor = [UIColor redColor];
                }

                [view addSubview:_gongliLabel];
            }
            
            UIView *view11 = [[UIView alloc] initWithFrame:CGRectMake(10, 50, 80, 0.7)];
            view11.backgroundColor = [UIColor colorWithRed:177/255.0 green:177/255.0 blue:177/255.0 alpha:1];
            [view addSubview:view11];
            
            if (i==5 || i == 7) {
                if (i ==5) {
                    _chaoTimeLabel = [[UILabel alloc] initWithFrame:CGRectMake(30, 13+15*(i-1)/2+10, 60, 15)];
                    _chaoTimeLabel.adjustsFontSizeToFitWidth = YES;
                    _chaoTimeLabel.textColor = [UIColor redColor];
                     _chaoTimeLabel.font = [UIFont systemFontOfSize:14];
                    [view addSubview:_chaoTimeLabel];
                }else {
                    _chaoGongliLabel = [[UILabel alloc] initWithFrame:CGRectMake(30, 13+15*(i-1)/2+10, 60, 15)];
                    _chaoGongliLabel.adjustsFontSizeToFitWidth = YES;
                     _chaoGongliLabel.textColor = [UIColor redColor];
                     _chaoGongliLabel.font = [UIFont systemFontOfSize:14];
                    [view addSubview:_chaoGongliLabel];
                }
            }
...
- (void)getDetailTimeWithTimestamp:(NSInteger)timestamp{
    ...
    NSString * s1 = _detailsDict[@"xcxq"][@"genche_chaoshi"];
    ...
    _timeLabel.text = [NSString stringWithFormat:@"%@:%@:%@",a,b,c];
    
    if (hour >= [s1 intValue]) {
        if (minute >= 10) {
            _chaoTimeLabel.text = [NSString stringWithFormat:@"%ld 小时",hour-[s1 integerValue]+1];
        }else {
            _chaoTimeLabel.text = [NSString stringWithFormat:@"%ld 小时",hour-[s1 integerValue]];
        }
...
-(void)getKmTamp:(NSInteger)timetamp {
    ...
    NSArray *newArray = [NSArray arrayWithArray:_detailsDict[@"xcxq"][@"carlist"]];
    if (newArray.count > 0) {
     NSString    *carids = newArray[0][@"carid"];
       NSString  *userids1 = newArray[0][@"userid"];
    
    
        [HttpManager postData:@{@"userid":userids1,@"id":_postIdString,@"carid":carids} andUrl:@"http://wx.leisurecarlease.com/tc.php?op=maps_list" success:^(NSDictionary *fanhuicanshu) {
              wange =0.00;
            ...
            NSLog(@"%.2f",wange);
            _gongliLabel.text = [NSString stringWithFormat:@"%.2f",(wange/1000)];
            
            NSString * s = _detailsDict[@"xcxq"][@"genche_chaogongli"];
            
            float  a = (wange/1000)-[s floatValue];
            
            if (a<=0) {
                a = 0;
            }
            _chaoGongliLabel.text = [NSString stringWithFormat:@"%.1f",a];
