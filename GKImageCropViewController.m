...
- (void)_setupNavigationBar{
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"确定", @"")
...
- (void)_setupUseButton{
    ...
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1) {
        ...
        [self.useButton setTitle:NSLocalizedString(@"确定",@"") forState:UIControlStateNormal];
...
//    CGFloat height = self.view.frame.size.height;
    ...
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(width, width*0.15), YES, 0.0);
	
    CGContextRef ctx = UIGraphicsGetCurrentContext();
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    CGGradientRef gradient = CGGradientCreateWithColorComponents(colorSpace, components, NULL, 2);
	
    CGContextDrawLinearGradient(ctx, gradient, CGPointMake(0, 0), CGPointMake(0, width*0.15), kCGImageAlphaNoneSkipFirst);
...
- (void)viewWillLayoutSubviews{
    ...
    self.toolbar.frame = CGRectMake(0, CGRectGetHeight(self.view.frame) -self.view.frame.size.width*0.15, self.view.frame.size.width, self.view.frame.size.width*0.15);
}
