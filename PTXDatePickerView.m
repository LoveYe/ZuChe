...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    if (self = [super initWithCoder:aDecoder]) {
        _minYear = 1970;
        _visible = NO;
        _datePickerViewShowModel = PTXDatePickerViewShowModelYearMonthDay;
...
- (void)__initView {
    ...
    cancle.frame = CGRectMake(0, 0, 60, 30);
    [cancle setTitle:@"取消" forState:UIControlStateNormal];
    UIColor *color = Color(7, 187, 177);
    [cancle setTitleColor:color forState:UIControlStateNormal];
    [cancle addTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *cancelButton = [[UIBarButtonItem alloc]initWithCustomView:cancle];
    ...
    UIButton *complet = [UIButton buttonWithType:UIButtonTypeCustom];
    complet.frame = CGRectMake(0, 0, 60, 30);
    [complet setTitle:@"完成" forState:UIControlStateNormal];
    [complet setTitleColor:color forState:UIControlStateNormal];
    [complet addTarget:self action:@selector(completion) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *completionButton = [[UIBarButtonItem alloc]initWithCustomView:complet];
