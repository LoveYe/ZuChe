...
@interface SWAddressViewConTroller ()<MAMapViewDelegate,AMapSearchDelegate, UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate>{
...
@implementation SWAddressViewConTroller
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    addressEnd = [NSString stringWithFormat:@"%@%@",poi.address,poi.name];
    
    CLGeocoder *geocoder=[[CLGeocoder alloc]init];
    //判断是否为空
    if ([self.startAddress isEqualToString:@"请点击添加"]) {
        
        if (_delegate && [_delegate respondsToSelector:@selector(dituSendMessage2:PriceTag:)]) {
            
            [_delegate dituSendMessage2:addressEnd PriceTag:0];
        }
        ...
        [geocoder geocodeAddressString:[NSString stringWithFormat:@"上海市%@",self.startAddress] completionHandler:^(NSArray<CLPlacemark *> * _Nullable placemarks, NSError * _Nullable error) {
            ...
            //            self.startPoint = [AMapNaviPoint locationWithLatitude:xxx longitude:yyy];
            //            self.endPoint   = [AMapNaviPoint locationWithLatitude:centerCoordinate.latitude longitude:centerCoordinate.longitude];
...
// 查询是否实现传地址的代理方法
//-(void)sendAddressString:(NSString *)string{
//
//    if (_delegate && [_delegate respondsToSelector:@selector(postAddressString: priceTag:)]) {
//
//        [_delegate postAddressString:string priceTag:price];
//    }
//    [self.navigationController popViewControllerAnimated:YES];
//}
...
- (void)onRouteSearchDone:(AMapRouteSearchBaseRequest *)request response:(AMapRouteSearchResponse *)response{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(dituSendMessage2:PriceTag:)]) {
        ...
        [_delegate dituSendMessage2:addressEnd PriceTag:a];
    }
    [self.navigationController popViewControllerAnimated:YES];
    
    //    NSLog(@"%@",response.route.destination);
    //    AMapNaviInfo *info = [[AMapNaviInfo alloc] init];
    //    NSString *str = info.currentRoadName;
    //    NSInteger a = info.routeRemainDistance;
    //    NSLog(@"%@ --- %ld",str,(long)a);
}
