...
@interface ShangwuOutView()<UIPickerViewDataSource,UIPickerViewDelegate>{
...
+(instancetype)makeViewWithMaskDatePicker:(CGRect)frame setTitle:(NSString *)title Arr1:(NSArray *)arr Arr2:(NSArray *)arr4{
    ...
    ShangwuOutView *mview = [[self alloc]initWithFrame:frame];
...
-(void)makeSureBtn:(UIButton *)makeSureBtn{
    ...
    if ([_delegate respondsToSelector:@selector(getChaochuChoice:)]) {
        
        [_delegate getChaochuChoice:allUser];
    }
    [self removeView];
}
//取消蒙层
