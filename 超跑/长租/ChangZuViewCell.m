...
@implementation ChangZuViewCell{
...
- (void)shoucangClick:(UIButton *)sender{
    ...
    if ([[ZCUserData share].userId isEqualToString:@""] || [ZCUserData share].userId == nil) {
        ...
        if (shoucang.selected == NO) {
            ...
            NSDictionary *dictionary = @{@"userid":[ZCUserData share].userId,@"carid":_model[@"carid"],@"model":@"insert",@"cartype":@"5"};
            ...
            [HttpManager postData:dictionary andUrl:@"http://wx.leisurecarlease.com/api.php?op=shoucang" success:^(NSDictionary *fanhuicanshu) {
                ...
                if (_sendDelegate && [_sendDelegate respondsToSelector:@selector(sendTag:)]) {
                    
                    [_sendDelegate sendTag:_index];
                }
            ...
            NSDictionary *dictionary = @{@"userid":[ZCUserData share].userId,@"carid":_model[@"carid"],@"model":@"delete",@"cartype":@"5"};
