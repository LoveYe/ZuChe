...
#import "ChangZuViewCell.h"
#import "CarDetailInfoView.h"

#define  PATH_URL @"http://wx.leisurecarlease.com/api.php?op=api_zclxlb"

@interface ChangzuViewController ()<UITableViewDataSource,UITableViewDelegate,YSLTransitionAnimatorDataSource,ShaiXuanDelegate,SendTag>{
...
//    NSMutableArray *_dataArray;
    ...
    ChangZuViewCell *cell;
}
@property (nonatomic , strong)NSMutableArray *dataArray;

@end

@implementation ChangzuViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell = [tableView dequeueReusableCellWithIdentifier:stac];
    if (!cell) {
        
        cell = [[ChangZuViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:stac];
...
//    NSMutableDictionary *model = [[NSMutableDictionary alloc] initWithDictionary:_dataArray[indexpath]];
//    [model setObject:@"1" forKey:@"shoucang"];
//    
//    [_dataArray removeObjectAtIndex:indexpath];
//    [_dataArray insertObject:model atIndex:indexpath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://wx.leisurecarlease.com%@",_dataArray[indexPath.row][@"cartu"]]];
    //    [view.carImageView sd_setImageWithURL:url completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
    //
    //    }];
    
    // [[view.view layer] addAnimation:animation forKey:@"rippleEffect" ];
    
    //    view.carImageView = .......
    
    [self.navigationController pushViewController:view animated:YES];
    
    //    [view setModalTransitionStyle:UIModalTransitionStylePartialCurl];
}
...
- (void)shoucang:(UIButton *)sender{
    ...
    if (isClick) {
        ...
        [sender setBackgroundImage:[UIImage imageNamed:@"收藏1.png"] forState:UIControlStateNormal];
        isClick = NO;
    }else{
