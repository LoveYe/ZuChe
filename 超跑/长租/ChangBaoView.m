...
@interface ChangBaoView ()<addressDelgate>{
...
- (void)createChangbao{
    ...
    scrollView.contentSize = CGSizeMake(0, width*3.05);
    ...
    UIView *carInfoMation = [[UIView alloc] initWithFrame:CGRectMake(width*0.05, CGRectGetMaxY(carInfoLabel.frame)+width*0.03, width*0.9, width*0.38)];
...
- (void)tijiaoINFO{
    ...
    if ([time isEqualToString:@"请设置日期"] || [time isEqualToString:@""]) {
        ...
        [HttpManager postData:dict1 andUrl:@"http://wx.leisurecarlease.com/api.php?op=api_tjczindent" success:^(NSDictionary *fanhuicanshu) {
            ...
            if ([self.jiedan intValue] == 0) {
                ...
                view.jiage = [NSString stringWithFormat:@"%d",[self.carprice intValue]/2];
