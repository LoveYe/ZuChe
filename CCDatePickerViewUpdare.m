...
@interface CCDatePickerViewUpdate ()
...
@implementation CCDatePickerViewUpdate
...
-(void)setupUI
{
    ...
    UIColor *color = [UIColor colorWithRed:7/255.0 green:187/255.0 blue:177/255.0 alpha:1.0];
    
    self.dateView=[[UIView alloc] initWithFrame:CGRectMake(W(30), 120, ScreenWidth-W(60), H(550))];
...
//    [self.okButton setTintColor:RGBA(210, 55, 61, 1.0)];
    ...
    UIView *aview = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.dateView.size.width*4/7+5, self.dateView.size.height-33)];
    [self.dateView addSubview:aview];

    
}
-(void)setInputString:(NSString *)inputString {
    _inputString = inputString;
    NSInteger a = [_inputString integerValue];
    
    [self getNowDate:[NSDate dateWithTimeIntervalSince1970:a] animated:YES];
 //   [self defaultConfig];
...
-(void)addLabelWithName:(NSArray *)nameArr {
    for (int i=0; i<nameArr.count; i++) {
        ...
        line.backgroundColor=RGBA(7, 187, 177, 1.0);
