...
@interface CarYearView() <UIPickerViewDataSource,UIPickerViewDelegate>
...
{
    CarYearView *mview = [[self alloc]initWithFrame:frame];
...
{
    //代理传值
    if ([_delegate respondsToSelector:@selector(getColorChoiceValues11:)]) {
        if (!self.colorStr) {
            self.colorStr = self.firstColor;
        }
        [_delegate getColorChoiceValues11:self.colorStr];
    }
    [self removeView];
}
//取消蒙层
