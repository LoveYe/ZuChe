...
@interface NewMapController2 ()<MAMapViewDelegate,AMapSearchDelegate, UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate>{
...
@implementation NewMapController2
...
-(UITableView *)tableView  {
    ...
    if (_tableView == nil) {
        ...
        _tableView=[[UITableView alloc]initWithFrame:CGRectMake(0, screen_width*0.83, screen_width, screen_height-screen_width*(0.13+0.7)) style:UITableViewStylePlain];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [geocoder geocodeAddressString:[NSString stringWithFormat:@"%@",poi.name] completionHandler:^(NSArray<CLPlacemark *> * _Nullable placemarks, NSError * _Nullable error) {
        ...
        NSLog(@"-----------%f  %f",a,b);
        NSDictionary *dic = @{@"carid":self.carid,@"caradd":poi.name,@"jd":lau,@"wd":lon};
        
        [HttpManager postData:dic andUrl:@"http://wx.leisurecarlease.com/api.php?op=api_bcdizhi" success:^(NSDictionary *fanhuicanshu) {
            
            NSLog(@"%@",[NSString stringWithFormat:@"%@",[fanhuicanshu objectForKey:@"msg"]]);
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(dituSendMessage:)]) {
            
            [_delegate dituSendMessage:[NSString stringWithFormat:@"%@%@",poi.address,poi.name]];
        }
