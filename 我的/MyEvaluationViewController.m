...
#import "AllPages.pch"
@interface MyEvaluationViewController ()<UIScrollViewDelegate>
{
    UIButton *_give;
    UIButton *_get;
...
- (void)viewDidLoad {
    ...
    [self addTitleViewWithTitle:@"我的评价"];
    ...
    [_give setTitle:@"给车主的评价" forState:UIControlStateNormal];
    ...
    [_get setBackgroundColor:[UIColor whiteColor]];
    [_get setTitle:@"来自租客的评价" forState:UIControlStateNormal];
...
-(void)viewController
{
    ...
    GiveJudgeViewController*give =[[GiveJudgeViewController alloc]init];
    give.view.frame = CGRectMake(0, 0, ScreenWidth, ScreenHeight);
    [_scrollview addSubview:give.view];
    [self addChildViewController:give];
    
    GetJudgeViewController *get =[[GetJudgeViewController alloc]init];
