...
@interface SexView() <UIPickerViewDataSource,UIPickerViewDelegate>
...
{
    SexView *mview = [[self alloc]initWithFrame:frame];
...
{
    //代理传值
    if ([_delegate respondsToSelector:@selector(getColorChoiceValues33:)]) {
        if (!self.colorStr) {
            self.colorStr = self.firstColor;
        }
        [_delegate getColorChoiceValues33:self.colorStr];
    }
    [self removeView];
}
//取消蒙层
