...
@implementation CarDetailInfoCell{
...
//    [self.contentView addSubview:star15];
    ...
    riliLabel = [[UILabel alloc] init];
    [self.contentView addSubview:riliLabel];
    
    riliView = [[UIView alloc] init];
    [self.contentView addSubview:riliView];
    
    jiantou3 = [[UIImageView alloc] init];
    [self.contentView addSubview:jiantou3];
...
//    star15.image = [UIImage imageNamed:@"五角星.png"];
    ...
    xian2.backgroundColor = Color(237, 237, 237);
        ...
        jieshao4.text = @"超出1小时10公里按超出标准计费";
...
//    bigView2.backgroundColor = [UIColor greenColor];
    ...
    jiantou3.image = [UIImage imageNamed:@"右(1).png"];
    
//    riliLabel.text = @"查看可租日期";
    riliLabel.textAlignment = NSTextAlignmentLeft;
    riliLabel.adjustsFontSizeToFitWidth = YES;
    riliLabel.textColor = Color(77, 77, 77);
    riliLabel.font = [UIFont systemFontOfSize:17];
    
    
    fuwuCenter.text = @"服务中心";
    fuwuCenter.textAlignment = NSTextAlignmentLeft;
    fuwuCenter.textColor = Color(77, 77, 77);
    fuwuCenter.adjustsFontSizeToFitWidth = YES;
...
- (void)layoutSubviews{
    ...
    xian3.frame = CGRectMake(0, CGRectGetMaxY(jieshao4.frame)+width*0.05, width, 1);
...
//        bigView1.frame = CGRectMake(0, CGRectGetMaxY(xian3.frame), width, width*0.12);
//        
//        pingjia.frame = CGRectMake(width*0.05, width*0.01, width*0.5, width*0.1);
//        
//        jiantou1.frame = CGRectMake(width*0.89, width*0.03, width*0.06, width*0.06);
//        
//        xian4.frame = CGRectMake(0, CGRectGetMaxY(bigView1.frame), width, 1);
//
//        riliView.frame = CGRectMake(0, CGRectGetMaxY(xian4.frame), width, width*0.12);
////        UITapGestureRecognizer *tapGester = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(riliClick:)];
////        [riliView addGestureRecognizer:tapGester];
//        
//        riliLabel.frame = CGRectMake(width*0.05, CGRectGetMaxY(xian4.frame)+width*0.02,width*0.3, width*0.1);
//        
////        jiantou3.frame = CGRectMake(width*0.89, CGRectGetMaxY(xian4.frame)+width*0.04, width*0.06, width*0.06);
//        
//        ditu.frame = CGRectMake(0, CGRectGetMaxY(riliView.frame), width, width*0.6);
...
//        yuding.frame = CGRectMake(0, CGRectGetMaxY(xiangsi.frame)-width*0.1, width, width*0.1);
//
//        [yuding addTarget:self action:@selector(buttonClick) forControlEvents:UIControlEventTouchUpInside];
...
//    newView.pinColor = BMKPinAnnotationColorRed;
...
-(UIView *)carousel:(iCarousel *)carousel viewForItemAtIndex:(NSInteger)index reusingView:(UIView *)view{
    ...
    if (dict3.count == 0) {
        ...
        UILabel *wuxiangsi = [[UILabel alloc] initWithFrame:CGRectMake(view.frame.size.width*0.05, view.frame.size.width*0.05, view.frame.size.width*0.9, view.frame.size.width*0.9*2/3)];
        wuxiangsi.text = @"暂无相似车源";
        wuxiangsi.textColor = Color(77, 77, 77);
        wuxiangsi.textAlignment = NSTextAlignmentCenter;
        wuxiangsi.adjustsFontSizeToFitWidth = YES;
        [view addSubview:wuxiangsi];
        
    }else{
        
        UIImageView *image = [[UIImageView alloc] init];
        
        [image sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://wx.leisurecarlease.com%@",[dict3 objectAtIndex:index][@"cartu"]]] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        ...
        image.frame = CGRectMake(view.frame.size.width*0.05, view.frame.size.width*0.05, view.frame.size.width*0.9, view.frame.size.width*0.9*2/3);
        [view addSubview:image];
        
        UIView *blackView = [[UIView alloc] initWithFrame:CGRectMake(0, image.frame.size.width*0.45, image.frame.size.width*0.4, image.frame.size.width*0.15)];
        ...
        UILabel *nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMinX(image.frame), CGRectGetMaxY(image.frame)+view.frame.size.width*0.05, view.frame.size.width*0.6, view.frame.size.width*0.1)];
        ...
        _wsStraRating = [[WSStarRatingView alloc] initWithFrame:CGRectMake(CGRectGetMinX(image.frame), CGRectGetMaxY(nameLabel.frame)+view.frame.size.width*0.01, view.frame.size.width*0.2125,view.frame.size.width*0.05) numberOfStar:5];
        ...
        __block CarDetailInfoCell *detaCell = self;
...
#pragma mark - 日历点击
- (void)riliClick:(NSString *)userid{
    
    NSLog(@"dianji  liri");
    if (_dicDelegate && [_dicDelegate respondsToSelector:@selector(riliSend:bukezuTime:)]) {
        
        [_dicDelegate riliSend:dict[@"carid"] bukezuTime:dict2[@"bukezu"]];
    }
}
