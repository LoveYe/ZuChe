...
@interface DituViewController ()<MAMapViewDelegate,AMapSearchDelegate, UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate>{
...
-(UIImageView *)middleImageView {
    ...
    if (_middleImageView == nil) {
        _middleImageView=[[UIImageView alloc]initWithFrame:CGRectMake(screen_width/2-17.5/2, (screen_width*0.7)/2-60, 17.5, 30)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MAMapPoint point = MAMapPointForCoordinate(CLLocationCoordinate2DMake(31.260503, 121.503311));
//    MAMapPointMake(31.260503, 121.503311);
    MAMapPoint point2 = MAMapPointForCoordinate(CLLocationCoordinate2DMake(xxx, yyy));
//    AMapLocationPoint *point = [];
    ...
    if (distance >= 15000) {
        ...
        if (distance > 25000) {
            ...
            if (distance >= 35000) {
                ...
                MAMapPoint point3 = MAMapPointMake(31.768882, 121.361436);
                //                BMKMapPointForCoordinate(CLLocationCoordinate2DMake(31.768882, 121.361436));
                CLLocationDistance distance1 = MAMetersBetweenMapPoints(point3, point2);
                if (distance1 <= 17000) {
                    self.abc = 1 ;
                    self.price = 3;
                }
    ...
    [self sendAddressString:[NSString stringWithFormat:@"%@",poi.name] price:self.price];
}
// 查询是否实现传地址的代理方法
-(void)sendAddressString:(NSString *)string price:(float)price{
