...
@interface NowLiulanViewController ()<UITableViewDataSource,UITableViewDelegate>
{
...
@implementation NowLiulanViewController
...
- (void)viewDidLoad {
    ...
    [self addTitleViewWithTitle:@"最近浏览"];
...
-(void)Downwork
{
    ...
    {
         [self xiaojiadeTishiTitle:@"对不起您还没有登录！"];
    }
    else
    {
        NSString *userid=[[NSString alloc]init];
        if ([ZCUserData share].isLogin) {
            userid=[ZCUserData share].userId;
        }else
        {
            userid=@"";
        }
        NSDictionary *dic=[NSDictionary dictionaryWithObjectsAndKeys:
                           userid,@"userid",nil];
        [HttpManager liuLanpostData:dic andUrl:ZUIJIN success:^(NSArray*arrayfanhui) {
            
            NSLog(@"+++++==%@",arrayfanhui);
            if (arrayfanhui.count==0) {
                
                [self xiaojiadeTishiTitle:@""];
                [self tishixiaoche];
            }else
            {
...
//        infor.carid=catidd;
//        NSString *listshop=[NSString stringWithFormat:@"%@",model.listshop];
